...
class ListDirectoryTool(BaseTool):
    ...
    parameters = [
        ...
        ToolParameter(
            name="sort_by",
            description="Field to sort by (name, type, size, modified)",
            required=False,
            default="name",
            type="string"
        ),
    ...
    def _search_file(self, file_path: Path, pattern: re.Pattern) -> List[dict]:
        ...
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                valid_sort_fields = ["name", "type", "size", "modified"]

                # Use default sorting if invalid sort_by field
                if sort_by not in valid_sort_fields:
                    sort_by = "name"

                # Use default sorting order if invalid sort_order
                if sort_order not in ["asc", "desc"]:
                    sort_order = "asc"

                # Handle None values for size (directories)
                if sort_by == "size":
                    sorted_contents = sorted(
                        contents,
                        key=lambda x: (x[sort_by] is None, x[sort_by] or 0),
